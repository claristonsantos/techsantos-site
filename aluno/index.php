...
const COURSE = [
  ...
  },
  {
    ...
    lessons: [
      {
        id: 'lab-01a', title: 'Estudo de Caso · Conectando e Importando Dados',
        diagram: '<div class="lab-flow"><div class="lab-flow-box">4 fontes<br><small>bagunçadas</small></div><div class="lab-flow-arrow">→</div><div class="lab-flow-box accent">Você conecta<br>e corrige cada uma</div><div class="lab-flow-arrow">→</div><div class="lab-flow-box">Prontas pra<br>limpeza</div></div>',
        steps: [
          { h: 'O desafio', p: 'Seu primeiro dia como analista de dados na TECH SANTOS BR: a gestora te repassa 4 arquivos com problemas diferentes de importação, cada um representando um jeito comum de fonte "suja" chegar até você. A partir daqui, nenhum passo a passo de clique — só o que cada arquivo final precisa satisfazer. Baixe um de cada vez e resolva no seu Power BI Desktop.' },
          { id: 'tipos-de-dados', h: 'Cadastro com tipos errados', p: 'Este cadastro chegou com datas e valores numéricos sendo lidos como texto pelo Power Query — se carregar assim, nenhuma fórmula de data ou soma vai funcionar depois.', arquivo: 'Tipos de Dados.xlsx',
            checklist: ['Todas as colunas de data reconhecidas como tipo Data, não texto', 'Colunas numéricas reconhecidas como Número, não texto', 'Consulta carregada sem nenhum erro de conversão de tipo'] },
          { id: 'tabela-ou-intervalo', h: 'Intervalo solto vs. Tabela nomeada', p: 'Esta planilha tem os dados como um intervalo comum, sem formatação de Tabela do Excel. Compare o que muda quando você conecta o Power Query direto no intervalo bruto, e quando primeiro transforma o intervalo em Tabela nomeada antes de conectar.', arquivo: 'Tabela ou Intervalo.xlsx',
            checklist: ['Uma versão da consulta carregada a partir do intervalo bruto', 'Uma versão carregada a partir de uma Tabela nomeada criada antes da conexão', 'Anotado qual das duas se comporta melhor quando novas linhas forem adicionadas depois'] },
          { id: 'importar-excel', h: 'Arquivo externo com várias abas', p: 'Este arquivo externo tem mais de uma planilha, mas só uma delas é a que sua análise precisa — trazer as outras junto só polui o modelo à toa.', arquivo: 'Importando um Arquivo Excel.xlsx',
            checklist: ['Apenas a aba correta carregada no modelo', 'Nenhuma aba extra ou desnecessária importada'] },
          { id: 'consulta-pasta', h: 'Sete relatórios separados numa pasta', p: 'A diretoria manda um relatório mensal solto — 7 arquivos com a mesma estrutura de colunas, um por mês. Ela quer decisões sobre o ano inteiro, não mês a mês separado.', arquivo: 'Consulta de Pasta (7 arquivos).zip',
            checklist: ['Os 7 arquivos combinados automaticamente em uma única tabela consolidada', 'Nenhum arquivo importado manualmente um por um'] },
        ],
      },
      {
        id: 'lab-02a', title: 'Estudo de Caso · Transformação e Limpeza de Dados',
        diagram: '<div class="lab-flow"><div class="lab-flow-box">Dados conectados<br><small>mas ainda sujos</small></div><div class="lab-flow-arrow">→</div><div class="lab-flow-box accent">17 correções<br>de limpeza</div><div class="lab-flow-arrow">→</div><div class="lab-flow-box">Pronta pro<br>modelo de BI</div></div>',
        steps: [
          { h: 'O desafio', p: 'Com as fontes já conectadas, sua gestora passa uma lista de 17 correções que a base final precisa ter antes de virar modelo de BI — cada uma testando uma técnica de limpeza diferente do Power Query, com seu próprio arquivo. De novo: o que precisa mudar em cada arquivo, não o caminho de cliques pra chegar lá.' },
          { id: 'preenchimento-colunas', h: 'Buracos deixados por células mescladas', p: 'Esta planilha veio de um relatório com células mescladas — toda vez que um valor se repetia, só a primeira linha ficou preenchida e o resto ficou vazio.', arquivo: 'Preenchimento de Colunas.xlsx',
            checklist: ['Nenhuma célula vazia na coluna afetada', 'Cada linha vazia preenchida com o último valor válido visto acima dela'] },
          { id: 'dividir-colunas', h: 'Uma coluna, duas informações', p: 'A coluna indicada guarda duas informações diferentes grudadas na mesma célula, separadas por um caractere — sua base precisa das duas em colunas distintas.', arquivo: 'Dividir Colunas.xlsx',
            checklist: ['Coluna original dividida em duas (ou mais) colunas distintas', 'Resultado comparado usando separação por delimitador e por número de caracteres fixo'] },
          { id: 'dividir-linhas', h: 'Vários valores numa célula só', p: 'Nesta planilha, uma única célula guarda uma lista de valores separados por vírgula — pra análise, cada valor precisa virar sua própria linha.', arquivo: 'Dividir Linhas.xlsx',
            checklist: ['Cada valor da lista virou uma linha separada', 'Restante da linha original repetido corretamente em cada nova linha'] },
          { id: 'colunas-personalizadas', h: 'Um cálculo que não existe na base', p: 'A base não tem uma coluna que sua análise precisa — ela tem que ser calculada a partir de duas colunas que já existem (por exemplo, preço vezes quantidade).', arquivo: 'Colunas Personalizadas.xlsx',
            checklist: ['Nova coluna personalizada criada com uma fórmula M válida', 'Resultado conferido manualmente em pelo menos 2 linhas'] },
          { id: 'coluna-exemplo', h: 'Ensinando o Power Query pelo exemplo', p: 'Em vez de escrever a fórmula você mesmo, digite o resultado esperado nas primeiras linhas e deixe o Power Query descobrir o padrão sozinho.', arquivo: 'Coluna de Exemplo.xlsx',
            checklist: ['Coluna criada a partir de exemplos, com a fórmula M gerada automaticamente', 'Fórmula gerada validada contra linhas que não foram usadas como exemplo'] },
          { id: 'mesclar-colunas', h: 'Duas colunas, um campo só', p: 'Sua base tem colunas de texto que, juntas, formam um único campo que a análise final precisa — como nome e sobrenome, ou rua e número.', arquivo: 'Mesclar Colunas.xlsx',
            checklist: ['Colunas de texto combinadas em um único campo', 'Separador entre os valores escolhido corretamente'] },
          { id: 'classificar-filtrar', h: 'Registros antigos demais no meio da base', p: 'A base tem registros de teste ou antigos demais misturados com os dados válidos — eles precisam sair antes de qualquer análise.', arquivo: 'Classificar e Filtrar.xlsx',
            checklist: ['Tabela ordenada por uma ou mais colunas relevantes', 'Registros antigos/de teste removidos por filtro'] },
          { id: 'formula-if', h: 'Classificar cada linha automaticamente', p: 'Sua gestora quer uma coluna que rotule cada linha como "Alto" ou "Baixo" com base em um valor numérico — sem fazer isso manualmente linha por linha.', arquivo: 'Fórmula IF.xlsx',
            checklist: ['Nova coluna com uma expressão condicional (if/then/else)', 'Classificação conferida em pelo menos 3 linhas de exemplo distintas'] },
          { id: 'formula-if-and', h: 'Duas condições ao mesmo tempo', p: 'Dessa vez a classificação só deve valer quando dois critérios diferentes forem verdadeiros ao mesmo tempo, não só um.', arquivo: 'Fórmula IF e AND.xlsx',
            checklist: ['Condição combinando dois testes lógicos', 'Resultado só marcado verdadeiro quando os dois critérios se cumprem juntos'] },
          { id: 'formula-adddays', h: 'Calcular uma data de vencimento', p: 'A base tem uma data de emissão, mas a análise precisa da data de vencimento — um número fixo de dias depois.', arquivo: 'Fórmula AddDays.xlsx',
            checklist: ['Nova coluna de data calculada a partir da data de emissão mais o número de dias'] },
          { id: 'formulas-texto', h: 'Nome digitado de qualquer jeito', p: 'Essa coluna de texto foi digitada manualmente, sem padrão — "joão", "João" e "JOÃO" aparecem como se fossem pessoas diferentes.', arquivo: 'Fórmulas de Texto.xlsx',
            checklist: ['Capitalização padronizada em toda a coluna', 'Variações do mesmo texto agora reconhecidas como idênticas'] },
          { id: 'coluna-dinamica', h: 'Virar categorias em colunas', p: 'Sua base tem uma coluna de categoria que precisa virar cabeçalhos de colunas separadas, com os valores agregados corretamente por categoria.', arquivo: 'Coluna Dinâmica - Pivot (3 arquivos).zip',
            checklist: ['Categorias da coluna original viraram cabeçalhos de novas colunas', 'Pelo menos duas funções de agregação testadas e comparadas (soma, contagem)'] },
          { id: 'transformar-colunas-linhas', h: 'Meses espalhados em colunas', p: 'O oposto do desafio anterior: os meses aparecem um por coluna nesta planilha, mas um modelo de BI precisa deles no formato longo, um por linha.', arquivo: 'Transformar Colunas em Linhas - Unpivot.xlsx',
            checklist: ['Colunas de mês convertidas em duas colunas (categoria e valor)', 'Tabela final no formato longo, pronta para um modelo de BI'] },
          { id: 'agrupar-por', h: 'Resumir sem perder o detalhe original', p: 'Sua gestora quer ver o total por vendedor (ou outra dimensão) sem precisar abrir a tabela detalhada toda vez.', arquivo: 'Agrupar Por.xlsx',
            checklist: ['Nova tabela resumida por uma dimensão escolhida', 'Agregação correta aplicada sobre as demais colunas'] },
          { id: 'acrescentar-consultas-1', h: 'Empilhando relatórios mensais', p: 'Um arquivo principal e três planilhas mensais (mesma estrutura de colunas) precisam virar uma única tabela consolidada, sem duplicar nem perder nenhuma linha.', arquivo: 'Acrescentar Consultas - Parte 1 (4 arquivos).zip',
            checklist: ['As 4 fontes empilhadas em uma única tabela consolidada'] },
          { id: 'acrescentar-consultas-2', h: 'Quando os nomes das colunas não batem', p: 'Desta vez as duas planilhas representam a mesma informação, mas com nomes de coluna divergentes — empilhar direto faria o Power Query tratá-las como colunas diferentes.', arquivo: 'Acrescentar Consultas - Parte 2 (2 arquivos).zip',
            checklist: ['Colunas com nomes divergentes renomeadas antes de acrescentar', 'Power Query reconhecendo as colunas como equivalentes depois do ajuste'] },
          { id: 'mesclar-consultas', h: 'Trazer dado de outra tabela', p: 'Sua base de vendas não tem o nome do vendedor, só um código — essa informação existe em outra tabela, que precisa ser cruzada pela chave em comum.', arquivo: 'Mesclar Consultas (4 arquivos).zip',
            checklist: ['Duas tabelas unidas por uma coluna-chave em comum', 'Pelo menos dois tipos de junção testados (interna e externa esquerda), observando o que acontece com linhas sem correspondência em cada caso'] },
        ],
      },
    ],
  },
