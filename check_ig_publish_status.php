...
$stmt = $pdo->query("SELECT id, tipo, status, agendado_para, meta_post_id, LEFT(email_ultimo_erro,80) AS erro FROM social_posts WHERE canal='instagram' AND agendado_para >= '2026-09-11' ORDER BY agendado_para");
foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $r) {
    echo "#{$r['id']} {$r['tipo']} {$r['status']} {$r['agendado_para']} meta={$r['meta_post_id']} erro={$r['erro']}\n";
}
