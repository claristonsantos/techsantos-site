...
// data_aula é salvo como horário local (America/Sao_Paulo, UTC-3) sem timezone, mas NOW() do
// MySQL roda em UTC nesse host — comparar direto adiantava os lembretes em 3h. DATE_SUB(NOW(),
// INTERVAL 3 HOUR) converte "agora" pro mesmo referencial naive do data_aula antes de comparar.
foreach($windows as[$label,$column,$minMinutes,$maxMinutes]){$stmt=$pdo->query("SELECT * FROM aulas_particulares_leads WHERE status='pago' AND data_aula IS NOT NULL AND {$column} IS NULL AND TIMESTAMPDIFF(MINUTE,DATE_SUB(NOW(),INTERVAL 3 HOUR),data_aula) BETWEEN {$minMinutes} AND {$maxMinutes}");foreach($stmt->fetchAll() as$lead){if(aulas_send_reminder($lead,$label)){$pdo->prepare("UPDATE aulas_particulares_leads SET {$column}=NOW(),email_ultimo_erro=NULL WHERE id=?")->execute([$lead['id']]);echo "aula {$lead['id']} {$label}: enviado\n";}else{$pdo->prepare('UPDATE aulas_particulares_leads SET email_ultimo_erro=? WHERE id=?')->execute(["Falha no lembrete {$label}",$lead['id']]);echo "aula {$lead['id']} {$label}: falha\n";}}}
