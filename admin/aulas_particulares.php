...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        ...
        if ($isSending || $isSendingPayment) {
            ...
            if($isSending&&!$linkReuniao&&!($googleCalendarConnected&&$scheduleDecision==='accepted'))$missing[]='link da reunião ou aceite do horário solicitado';
            ...
            if($isSending&&empty($lead['link_reuniao'])&&$googleCalendarConnected&&$scheduleDecision==='accepted'){
                $meetError=null;
...
<main class="admin-main" id="adminContent" tabindex="-1">
  ...
  <section class="admin-form-section"><div class="form-card"><div class="admin-form-section-head"><h2>Atualizar solicitação #<?= (int)$selected['id'] ?></h2><a href="/admin/aulas_particulares.php">Fechar</a></div>
    ...
    <form method="post" id="lessonProposalForm"><?= csrf_field() ?><input type="hidden" name="id" value="<?= (int)$selected['id'] ?>"><input type="hidden" name="action" id="proposalAction" value="save"><input type="hidden" name="schedule_decision" id="scheduleDecision" value="">
      ...
      <div class="field"><label for="link_reuniao">Link da reunião</label><input id="link_reuniao" name="link_reuniao" type="url" <?= empty($selected['link_reuniao'])?'required':'' ?> value="<?= htmlspecialchars($selected['link_reuniao']??'',ENT_QUOTES) ?>" placeholder="<?= $googleCalendarConnected?'Será criado após aceitar o horário e confirmar':'https://meet.google.com/...' ?>"><small style="color:var(--ink-soft)"><?php if($googleCalendarConnected): ?>Clique em <strong>Aceitar horário solicitado</strong>; o Meet será criado ao confirmar a aula.<?php else: ?>Preencha o link ou <a href="/admin/google_calendar_setup.php">conecte o Google Calendar</a>.<?php endif; ?></small></div><div class="field"><label for="observacoes">Mensagem do e-mail</label><textarea id="observacoes" name="observacoes" rows="9" required><?= htmlspecialchars($selected['observacoes'] ?: $defaultProposalMessage,ENT_QUOTES) ?></textarea><small id="emailMessageHelp" style="color:var(--ink-soft)">Este texto será enviado ao aluno junto com o resumo da aula.</small></div>
