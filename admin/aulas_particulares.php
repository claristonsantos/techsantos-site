...
<main class="admin-main" id="adminContent" tabindex="-1">
  ...
  <section class="admin-form-section"><div class="form-card"><div class="admin-form-section-head"><h2>Atualizar solicitação #<?= (int)$selected['id'] ?></h2><a href="/admin/aulas_particulares.php">Fechar</a></div>
    ...
    <form method="post"><?= csrf_field() ?><input type="hidden" name="id" value="<?= (int)$selected['id'] ?>">
      ...
      <div class="form-actions"><button class="btn btn-ghost on-light" type="submit" formnovalidate>Salvar rascunho</button><button class="btn btn-primary" type="submit" name="action" value="save_and_send">Salvar e enviar proposta + pagamento</button><a class="btn btn-ghost on-light" href="<?= htmlspecialchars(lesson_whatsapp($selected,$pricing),ENT_QUOTES) ?>" target="_blank" rel="noopener">Abrir WhatsApp</a></div>
