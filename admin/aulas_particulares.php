...
$googleCalendarConnected = google_calendar_is_connected($pdo);
$columns = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $scheduleDecision = (string)($_POST['schedule_decision'] ?? '');
    ...
    if ($id > 0) {
        ...
        if ($isSending || $isSendingPayment) {
            ...
            if($isSending&&!$linkReuniao&&!$googleCalendarConnected)$missing[]='link da reunião';
            ...
            if($isSending&&empty($lead['link_reuniao'])&&$googleCalendarConnected){
                $meetError=null;
                $googleEvent=google_calendar_create_lesson($pdo,$lead,$meetError);
                if(!$googleEvent){
                    $pdo->prepare('UPDATE aulas_particulares_leads SET email_ultimo_erro=? WHERE id=?')->execute(['Falha ao criar Google Meet: '.($meetError?:'erro não identificado'),$id]);
                    header('Location: /admin/aulas_particulares.php?editar='.$id.'&proposta=meet');exit;
                }
                $lead['link_reuniao']=$googleEvent['meet_link'];
                $lead['google_calendar_event_id']=$googleEvent['event_id'];
                $pdo->prepare('UPDATE aulas_particulares_leads SET link_reuniao=?,google_calendar_event_id=?,email_ultimo_erro=NULL WHERE id=?')->execute([$lead['link_reuniao'],$lead['google_calendar_event_id'],$id]);
            }
...
<main class="admin-main" id="adminContent" tabindex="-1">
  ...
  <section class="admin-form-section"><div class="form-card"><div class="admin-form-section-head"><h2>Atualizar solicitação #<?= (int)$selected['id'] ?></h2><a href="/admin/aulas_particulares.php">Fechar</a></div>
    ...
    <form method="post" id="lessonProposalForm"><?= csrf_field() ?><input type="hidden" name="id" value="<?= (int)$selected['id'] ?>"><input type="hidden" name="action" id="proposalAction" value="save"><input type="hidden" name="schedule_decision" id="scheduleDecision" value="">
      ...
      <div class="field"><label for="link_reuniao">Link da reunião</label><input id="link_reuniao" name="link_reuniao" type="url" <?= empty($selected['link_reuniao'])&&!$googleCalendarConnected?'required':'' ?> value="<?= htmlspecialchars($selected['link_reuniao']??'',ENT_QUOTES) ?>" placeholder="<?= $googleCalendarConnected?'Será criado automaticamente ao confirmar':'https://meet.google.com/...' ?>"><small style="color:var(--ink-soft)"><?php if($googleCalendarConnected): ?>O Meet é criado automaticamente ao confirmar a aula, com a data e hora preenchidas acima — mesmo sugerindo outro horário.<?php else: ?>Preencha o link ou <a href="/admin/google_calendar_setup.php">conecte o Google Calendar</a>.<?php endif; ?></small></div><div class="field"><label for="observacoes">Mensagem do e-mail</label><textarea id="observacoes" name="observacoes" rows="9" required><?= htmlspecialchars($selected['observacoes'] ?: $defaultProposalMessage,ENT_QUOTES) ?></textarea><small id="emailMessageHelp" style="color:var(--ink-soft)">Este texto será enviado ao aluno junto com o resumo da aula.</small></div>
