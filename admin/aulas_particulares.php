    ...
    if ($id > 0) {
        ...
        if ($isSending) {
            ...
            if (!aulas_send_proposal($lead)) {
                $pdo->prepare('UPDATE aulas_particulares_leads SET email_ultimo_erro=? WHERE id=?')->execute(['Falha ao enviar proposta',$id]);
                header('Location: /admin/aulas_particulares.php?editar='.$id.'&proposta=email'); exit;
            }
            $pdo->prepare("UPDATE aulas_particulares_leads SET pagamento_link=?,mercadopago_preference_id=?,proposta_enviada_em=NOW(),email_ultimo_erro=NULL,status='agendado' WHERE id=?")->execute([$payment['url'],$payment['id'],$id]);
            header('Location: /admin/aulas_particulares.php?editar='.$id.'&proposta=enviada'); exit;
        }
...
<main class="admin-main" id="adminContent" tabindex="-1">
  ...
  <section class="admin-form-section"><div class="form-card"><div class="admin-form-section-head"><h2>Atualizar solicitação #<?= (int)$selected['id'] ?></h2><a href="/admin/aulas_particulares.php">Fechar</a></div>
    ...
    <form method="post" id="lessonProposalForm"><?= csrf_field() ?><input type="hidden" name="id" value="<?= (int)$selected['id'] ?>"><input type="hidden" name="action" id="proposalAction" value="save">
      ...
      <div class="field"><label for="link_reuniao">Link da reunião</label><input id="link_reuniao" name="link_reuniao" type="url" required value="<?= htmlspecialchars($selected['link_reuniao']??'',ENT_QUOTES) ?>" placeholder="https://meet.google.com/..."></div><div class="field"><label for="observacoes">Mensagem do e-mail</label><textarea id="observacoes" name="observacoes" rows="9" required><?= htmlspecialchars($selected['observacoes'] ?: $defaultProposalMessage,ENT_QUOTES) ?></textarea><small style="color:var(--ink-soft)">Este texto será enviado ao aluno junto com o resumo e o botão de pagamento.</small></div>
      <div class="lesson-message" id="proposalClientMessage" role="alert"></div><div class="form-actions"><button class="btn btn-ghost on-light" type="submit" formnovalidate>Salvar rascunho</button><button class="btn btn-primary" type="button" id="sendProposalButton">Salvar e enviar proposta + pagamento</button><a class="btn btn-ghost on-light" href="<?= htmlspecialchars(lesson_whatsapp($selected,$pricing),ENT_QUOTES) ?>" target="_blank" rel="noopener">Abrir WhatsApp</a></div>
      <?php if(!empty($selected['proposta_enviada_em'])): ?><small style="display:block;margin-top:.7rem">Última proposta enviada: <?= date('d/m/Y H:i',strtotime($selected['proposta_enviada_em'])) ?></small><?php endif; ?>
    </form>
  </div></section>
  <?php endif; ?>
